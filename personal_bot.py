...
async def cmd_alert_ema(update, context):
    ...
    /alert_ema BTC weekly 200  - alert when BTC price crosses EMA200 weekly
    """
    uid  = update.effective_user.id
    user = get_user(uid)
    ...
    if len(context.args) < 3:
        await update.message.reply_text(
            "Usage:\n"
            "/alert_ema BTC daily 200\n"
            "/alert_ema BTC weekly 200\n\n"
            "Vei primi alerta cand pretul incruciseaza EMA-ul selectat."
        )
        return
    ...
    if timeframe not in ["daily", "weekly"]:
        await update.message.reply_text("Timeframe must be: daily or weekly")
        return
    ...
    alert_key = symbol + ":" + timeframe + ":" + str(period)
...
if __name__ == "__main__":
    main()
        ...
        "whales_title": "Tranzactii Balene (>$1M)",
        "no_whales": "Nu au fost detectate tranzactii mari recent.",
        "alert_rsi_set": "Alerta RSI setata: {} sub {}",
        "alert_fear_set": "Alerta Fear & Greed setata: sub {}",
        "alerts_empty": "Nu ai alerte active.",
        "pnl_empty": "Nu ai monede in portofoliu pentru P&L.",
    },
    "en": {
        ...
        "whales_title": "Whale Transactions (>$1M)",
        "no_whales": "No large transactions detected recently.",
        "alert_rsi_set": "RSI alert set: {} below {}",
        "alert_fear_set": "Fear & Greed alert set: below {}",
        "alerts_empty": "You have no active alerts.",
        "pnl_empty": "No coins in portfolio for P&L.",
    }
...
def get_rsi(slug, days=14):
    """Calculate RSI from CoinGecko OHLC data."""
    cached = cache_get("rsi:" + slug)
    if cached is not None:
        return cached
    try:
        data = cg_get("/coins/" + slug + "/ohlc", params={"vs_currency": "usd", "days": "30"})
        ...
        if avg_loss == 0:
            ...
            rsi = 100 - (100 / (1 + rs))
        rsi = round(rsi, 1)
        cache_set("rsi:" + slug, rsi)
        return rsi
    except Exception as e:
        logger.error("get_rsi error: " + str(e))
    return None
...
async def generate_report(uid):
    ...
    for symbol in all_coins[:5]:
        ...
        if rsi is not None:
            if rsi <= 30:
                signals.append(symbol + " RSI " + str(rsi) + " - Oversold 🟢")
            elif rsi >= 70:
                signals.append(symbol + " RSI " + str(rsi) + " - Overbought 🔴")
...
async def cmd_alert_rsi(update, context):
    uid  = update.effective_user.id
    user = get_user(uid)
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /alert_rsi BTC 30")
        return
    symbol    = context.args[0].upper()
    try:
        ...
        await update.message.reply_text("Invalid threshold.")
        return
    user["alerts"]["rsi"][symbol] = threshold
    save_data()
    await update.message.reply_text(t(uid, "alert_rsi_set", symbol, threshold))
...
async def cmd_alerts(update, context):
    ...
    if not rsi_alerts and not fear_alert:
        await update.message.reply_text(t(uid, "alerts_empty"))
        return

    lines = ["Your Alerts\n"]
    if rsi_alerts:
        lines.append("RSI Alerts:")
        for symbol, threshold in rsi_alerts.items():
            lines.append("  " + symbol + " < " + str(threshold))
    if fear_alert:
        lines.append("Fear & Greed Alert: < " + str(fear_alert))
    await update.message.reply_text("\n".join(lines))
            ...
            if rsi is not None and rsi <= threshold:
                try:
                    await context.bot.send_message(
                        ...
                        text=(
                            "RSI Alert - " + symbol + "\n\n"
                            "RSI: " + str(rsi) + " (below " + str(threshold) + ")\n"
                            "Possible oversold signal."
                        )
                    )
                except Exception as e:
                    logger.error("RSI alert error: " + str(e))
...
def main():
    ...
    app.add_handler(CommandHandler("watchlist",    cmd_watchlist))
    app.add_handler(CommandHandler("whales",       cmd_whales))
    app.add_handler(CommandHandler("alert_rsi",    cmd_alert_rsi))
    app.add_handler(CommandHandler("alert_fear",   cmd_alert_fear))
    app.add_handler(CommandHandler("alerts",       cmd_alerts))
    app.add_handler(CommandHandler("report",       cmd_report))
    app.add_handler(CommandHandler("set_report",   cmd_set_report))
